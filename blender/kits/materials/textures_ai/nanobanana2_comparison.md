...
| `tex_dirt_path` | `tex_dirt_path_nb2_v2.jpg` | REVIEW | Valid JPEG and tan path palette, but the regenerated file has a visible radial/X rut pattern; compare carefully before promotion. |
| `tex_paving` | `tex_paving_nb2_v2.jpg` | REVIEW | Clear flagstone read, but the regenerated file is more colorful and has heavier joints than the reference plaza. |
...
The retained Nano Banana 2 files were regenerated directly from the API after an earlier binary move corrupted the JPEG headers/bodies. They now validate as 1024x1024 JFIF JPEGs with macOS `sips` and Preview-compatible headers.
