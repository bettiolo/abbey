...
| Space (`RingBell()`) | Recall pulse: raises `BellRang(position, bellRadius)`. Recalls covered villagers with a speed boost, calms panic, calls a bonded hound. 5s cooldown. |
| F (`CarryFlame(bool)`) | Raise/douse a mobile `LightSource` child. Drains stamina per second while lit; self-extinguishes at 0 stamina. Toggle cooldown stops flickering. |
| E (`Rescue(villager)` / `ReleaseRescued()`) | Attach the nearest villager in rescued-follow; release inside Safe light to complete the rescue (`VillagerRescued`). |
| H (`FeedHound(hound)`) | Spend one carried food within interact range: hunger down, trust up (`HoundFed`). |
| F1 | Toggle the `Abbey.Debugging.DebugOverlay` panel (clock, villagers, hound bond, monsters, director, lights/resources). |
| `SetMoveTarget(worldPos)` API | Click-to-move style destination (also what tests drive). |

Key bindings live in `BellkeeperInputBindings`, thin glue over the controller APIs —
the controller itself stays input-agnostic so tests keep driving the methods
directly. Camera: WASD/arrows pan when the camera has no follow target (the
bootstrapped scene follows the hero), scroll wheel zooms orthographic size only.
...
- **Scenes are generated by code** (`PrototypeSceneBuilder`), never hand-authored;
  no `.unity` file is committed or required by any logic or test. The bootstrapper
  saves `Assets/_Game/Scenes/Prototype01.unity` locally for Play mode and builds
  (`Builds.BuildMacOS` regenerates it on demand when missing).
