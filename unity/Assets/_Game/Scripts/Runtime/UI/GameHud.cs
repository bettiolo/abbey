...
namespace Abbey.UI
{
    ...
    public class GameHud : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            if (!visible)
            {
                GUI.Label(new Rect(Screen.width * 0.5f - 40f, 8f, 200f, 22f),
                    $"[{toggleKey}] HUD");
                return;
            }
        ...
        void DrawTopStrip()
        {
            ...
            var strip = new Rect(x, 8f, width, 52f);
            ...
            GUI.Label(new Rect(strip.x + 10f, strip.y + 4f, 300f, 20f), clockText, _labelStyle);
            GUI.Label(new Rect(strip.x + strip.width - 260f, strip.y + 4f, 250f, 20f),
                headcount, _labelStyle);

            // Thin phase-progress bar under the clock text.
            if (clock != null)
            {
                DrawBar(new Rect(strip.x + 10f, strip.y + 24f, 180f, 5f),
                    clock.PhaseProgress, PhaseBarColor(clock.Phase));
            }

            GUI.Label(new Rect(strip.x + 10f, strip.y + 31f, strip.width - 20f, 20f),
                FormatStockLine(
        ...
        void DrawHeroStrip()
        {
            ...
            float width = 480f;
            float height = 30f;
            var strip = new Rect((Screen.width - width) * 0.5f,
            ...
            DrawBar(new Rect(strip.x + 36f, y, 110f, 12f),
                cfg.bellkeeperMaxHealth > 0f ? _hero.Health / cfg.bellkeeperMaxHealth : 0f,
                new Color(0.85f, 0.30f, 0.25f));
            GUI.Label(new Rect(strip.x + 156f, strip.y + 5f, 24f, 20f), "ST", _labelStyle);
            DrawBar(new Rect(strip.x + 182f, y, 110f, 12f),
                cfg.bellkeeperMaxStamina > 0f ? _hero.Stamina / cfg.bellkeeperMaxStamina : 0f,
                new Color(0.35f, 0.65f, 0.85f));
            GUI.Label(new Rect(strip.x + 302f, strip.y + 5f, 170f, 20f),
